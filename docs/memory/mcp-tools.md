...
!!! tip "Primary use case: conversation context extender"
    Call `memory_recall("project context")` at the start of every new chat. Get a complete project briefing — 30 days of decisions and conventions — in under 200 tokens. No more re-pasting architecture docs. See the [AI Coding Agent guide](use-cases/coding-agent.md) for the full session workflow.
...
- [AI Coding Agent guide](use-cases/coding-agent.md) — full session workflow, CLAUDE.md template, TOUCHES bridge
- [Concepts & data model](concepts.md) — bi-temporal schema, recall pipeline, scoring formula
- [CLI reference](cli.md) — `init`, `stats`, `search`, `show`, `forget`, `export`, `consolidate`, `import`
- [Knowledge Graph MCP Server](../integrations/mcp-server.md) — code graph tools: `query_codebase`, `graph_status`, `list_entities`
